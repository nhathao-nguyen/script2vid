...
export default function Gallery({ segments, onSelectOption, onToggleManualReview, filter, isProcessing, scrollParentRef }: GalleryProps) {
  const [scroller, setScroller] = useState<HTMLElement | null>(null);

  useEffect(() => {
    if (scrollParentRef?.current) {
      setScroller(scrollParentRef.current);
    }
  }, [scrollParentRef]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-24 h-full">
      {scroller ? (
        <Virtuoso
          customScrollParent={scroller}
          data={sortedSentenceIds}
          itemContent={(index, sentenceId) => {
            const group = sentenceGroups[sentenceId];
            return (
              <div key={sentenceId} className="space-y-12 pb-24">
